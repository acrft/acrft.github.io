...
"ربط الديسكورد 💬🔗",
"إسقاط رؤوس اللاعبين 💀",
"أوامر السيرفر الأساسية ⚙️",
"الدخول بدون حساب رسمي 🌐",
"دعم بدروك مع الجافا 📱🖥️",
"إدارة السكنات 🎨",
"حماية الأراضي 🛡️",
"نظام الصلاحيات 🔑",
"تعدد العوالم 🌍",
...
"الجلوس 🪑",
"تغيير السكينات 👕",
"قطع الأشجار بضربة واحدة 🌳🪓",
"نظام الاقتصاد 💰",
"تعدين العروق ⛏️",
"دعم الإصدارات الأقدم ⏪",
"دعم الإصدارات القديمة جدًا ⏮️",
"دعم جميع الإصدارات 🔄",
"إنشاء عالم فارغ 🕳️🌍",
"تعديل العالم 🛠️",
"حماية المناطق 🚧",
"شخصيات NPC 🤖"
    ...
    en: {
        ...
        adminsTitle: "👑 Staff",
        adminsNames: "Alam Gamer ☘️, Lost Fox 🦊, King Snow 👑❄️",
...
function setOfflineUI(dot, text, pNum, pMax, ping) {
    dot.className = "status-dot dot-offline";
    text.innerText = translations[currentLang].serverOffline + " ❌";
    if (pNum) pNum.innerText = "0";
    if (pMax) pMax.innerText = "0";
    if (ping) {
        ping.innerText = "--";
        ping.style.color = "inherit";
    }
}
...
async function updateServerStatus() {
    ...
    // 1. We declare 'data' out here so the whole function can access it
    let data = null;

    try {
        const response = await fetch(`https://api.mcsrvstat.us/2/${serverIP}:28303?t=${Date.now()}`);

        // Check if the response actually worked
        if (!response.ok) {
        ...
        if (isActuallyOnline) {
            ...
            // Set offline state
            setOfflineUI(statusDot, statusText, playerNum, maxPlayers, pingDisplay);
        }
    } catch (error) {
